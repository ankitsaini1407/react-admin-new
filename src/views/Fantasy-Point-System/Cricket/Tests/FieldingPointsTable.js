...
const CricketTestsFieldingPoints = () => {
  ...
  const getData = async () => {
    try {
      await axios
        .get(`${get_fantasy_cricket_points}?gameType=tests&action_name=fielding`, {
        ...
        .then((response) => {
          if (response) {
            ...
            response.data.data.map((elem) => {
              ...
              setFilteredData(elem.actionPoints);
            });
...
export default CricketTestsFieldingPoints;
