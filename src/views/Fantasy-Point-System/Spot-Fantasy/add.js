...
const AddSpotFantasyPoints = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, action) => {
      if (imageError == "") {
        ...
        await axios
          .post(`${add_download_banner}?subType=spot-fantasy-points`, formData, {
            headers: { token: Cookies.get("token") },
          })
          .then((response) => {
            if (response) {
              navigate("/fantasy-point-system/spot-fantasy", { state: { active_table: "3" } });
  ...
  const checkImage = (e) => {
    ...
    e.target.naturalWidth === 900 && e.target.naturalHeight === 1455
      ? ""
...
export default AddSpotFantasyPoints;
