...
const DownloadRatingImage = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, action) => {
      if (imageError == "") {
        ...
        await axios
          .post(`${add_download_banner}?subType=rating`, formData, {
  ...
  const checkImage = (e) => {
    ...
    e.target.naturalWidth === 573 && e.target.naturalHeight === 220
      ? ""
  ...
  return (
    <Form onSubmit={formik.handleSubmit}>
      <Form.Group className="mb-3" controlId="formBasicEmail">
        <Form.Label>Rating Image</Form.Label>
        <Form.Control
          type="file"
...
export default DownloadRatingImage;
