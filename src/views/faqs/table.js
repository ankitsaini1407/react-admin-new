...
import { get_faqs, update_status_faqs } from "../../utils/APIRoutes";
...
const Faqs = () => {
  ...
  const getData = async () => {
    try {
      await axios.get(`${get_faqs}?page=${pageNumber-1}&size=${perPage}`, { headers: { token: Cookies.get("token") } })
      .then(response => {
        if (response) {
  ...
  const handleChange = (id, active) => async (e) => {
    active = !active;
    let response = await axios.post(`${update_status_faqs}?id=${id}&isActive=${active}`);
    getData();
  };
  ...
  return (
    <div className="container">
      <DataTable
        ...
        actions={<Link to="/faqs/add"><button data-toggle="modal" data-target="#myModal" className="btn btn-sm btn-success">ADD+</button></Link>}
