...
import {
  get_about_us_cms_route,
  change_about_us_cms_status_route,
  delete_about_us_cms_route,
...
const AboutUsCms = () => {
  ...
  const getData = async () => {
    await axios
      .get(
        `${get_about_us_cms_route}?type=about-us&page=${
          pageNumber - 1
        }&size=${perPage}`,
  ...
  const handleChange = (id, active) => async (e) => {
    active = !active;
    await axios.post(
      `${change_about_us_cms_status_route}?id=${id}&isActive=${active}`
    );
    getData();
  };

  const handleSlug = (slug, description) => async () => {
    navigate(`/about-us/cms/${slug}`, { state: { description: description } });
  };

  const handleDelete = (id) => async (e) => {
    await axios
      .delete(`${delete_about_us_cms_route}?id=${id}`)
      .then((response) => {
        getData();
        if (response) {
  ...
  const handleEdit = (id, type, title, description) => async () => {
    navigate(`/about-us/cms/edit/${id}`, {
      state: { id: id, type: type, title: title, description: description },
    });
  };
