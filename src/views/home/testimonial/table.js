...
const HomeWinners = () => {
  ...
  const getData = async () => {
    try {
      await axios.get(`${get_testimonial_route}?type=home`, { headers: { token: Cookies.get("token") } })
        .then(response => {
          if (response) {
  ...
  return (
    <div className="container">
      <DataTable
        ...
        actions={<Link to="/home/testimonial/add"><button data-toggle="modal" data-target="#myModal" className="btn btn-sm btn-success">ADD+</button></Link>}
